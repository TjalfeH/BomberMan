function ChatPanel( game, context ){
	...
	var instruction_label = game.add.text(this.bg.width - 20, 5, "BombSaland.nl - Free Bomberman Multiplayer - BETA", { font: "18px Arial", fill: "#FFFF99" } );
...
function PlayerPreview( game, id, name, serial, frags, color, self = false ){
	...
	var name_label = game.add.text(bm_body.width + 10, 1, name, font_style);
	this.add(name_label);
            ...
            mute_btn.key==='soundon'?mute_btn.loadTexture('soundoff',0):mute_btn.loadTexture('soundon',0);
				...
				if(name.indexOf("Guest*") == 0){
					...
					reg.events.onInputDown.add(function(){
						window.location.href = "http://www.BombSaland.nl";
					}, this);
				}
    }

	font_style.fontWeight = "normal";
	font_style.fontSize = 18;
	var points_label = game.add.text(bm_body.width + 11, 40, "POINTS: 0", font_style);
	this.add(points_label);

	font_style.wordWrap = true;
	font_style.wordWrapWidth = Retoosh.WIDTH - Retoosh.HEIGHT - 60;
	var message_label = game.add.text(0, 64, "", font_style);
