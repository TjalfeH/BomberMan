...
function SocketClient( context ){
	...
	socket.on("player in fire", this.onBombermanInExplosion); // when player is in fire.
...
SocketClient.prototype.onMapChange = function( map_data ){
	console.log("Socket event 'map change'");
...
SocketClient.onBombermanInExplosion = function(bomberman, explosion){
