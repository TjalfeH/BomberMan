function SocketHandler( context ){
	...
	this.onPlayerSpawn = function( player_data ){
		...
		var nickname = game.add.text(0, 0, player_data.nickname, { font: "22px Arial", fill: "#"+Phaser.Color.componentToHex(bomberman.color) });
		nickname.anchor.set( 0.5, 1 );
		context.nicknames[player_data.serial] = nickname;
	...
	this.onPlayerPlantBomb = function( bomb_data ){
		...
		// start bomb countdown timer if host (there are so many function depending on the host...) (problem is that when a host is inactive, the update 
		// won't be called that often anymore, most likely rendering the countdown invalid then) - instead of a count down we could maybe set a time when the bomb detonates
		// or we simply sync up the countdown too when transitioning hosts, however, this is difficult because when we transition host, the current host already lost connection
		if(IS_HOST) 
		bomb.startCountdown( CONFIG.bombs_countdown );
	};
	...
	this.onBombExplode = function( bomb_data ){
		...
		//context.players.map(console.log);
		context.players.map(p => console.log(`serial ${p.serial} is_dying ${p.is_dying} is_dead ${p.is_dead} is_infire ${p.is_infire} is_invincible ${p.is_invincible} i_countdown ${p.i_countdown} i_timestamp ${p.i_timestamp} \n`));
