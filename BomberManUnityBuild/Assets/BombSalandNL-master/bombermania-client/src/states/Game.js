...
Retoosh.Game.prototype = {
    ...
    create: function() {
		...
		*/
		...
		for(var i = 0; i < this.room.max_players; i++){
			...
			if(player_data){
				...
				bomberman.is_active = player_data.is_active;
		...
		*/
		...
		this.chatKey = game.input.keyboard.addKey(Phaser.KeyCode.ENTER);
    ...
    update: function() {
		...
		//ToDo: potentially the issue why some characters are not killed: maybe not all of that state is transferred if the host changes?
		//if(IS_HOST) {
		...
		for (let p = 0; p < this.players.length; p++){
			if (this.players[p].visible && this.nicknames[p] != null){
				this.nicknames[p].visible = true;
				this.nicknames[p].x = this.players[p].x;
				this.nicknames[p].y = this.players[p].y - 20;
			} else {
				this.nicknames[p].visible = false;
			}
	...
	onBombermanInExplosion: function(bomberman, explosion){
		console.log(`in exlosion: ${bomberman.nickname} is_invincible ${bomberman.is_invincible} is_dead ${bomberman.is_dead} is_dying ${bomberman.is_dying} is_infire ${bomberman.is_infire}`)
		...
		if ((!bomberman.is_dying && !bomberman.is_dead) || (!bomberman.is_active)) {
